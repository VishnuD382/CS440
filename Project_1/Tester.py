...
Adaptive = False  # true for adaptive


lines = [[int(v) for v in line.split()]
         for line in open('02.txt')]  # insert appropriate directory of generated maze.
...
if reverse:
    ...
    pathSet = repeatedA.repeatedAStar(
        cols, openedSet, closedSet, end, start, test)
    ...
    if Adaptive:
        ...
        pathSet = adaptive.adaptiveAStar(
            cols, openedSet, closedSet, start, end, test)
    else:
        start_Timer = ClassNode.timeit.default_timer()
        ClassNode.heapq.heappush(test, (start.f, start))
        pathSet = repeatedA.repeatedAStar(
...
cmap = ClassNode.mpl.colors.ListedColormap(
